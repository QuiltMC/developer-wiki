...
public record WikiStructure(
		List<WikiType> libraries,
		List<WikiType> versions,
		String librariesNavbar,
		String versionsNavbar,
		String masterSidebar
) {
	public record WikiType(
			String name,
			String title,
			String content,
			String sidebar,
		...
		public static final class Builder {
			...
			private Path path;
			private final List<WikiSubEntry> wikis;
			...
			public WikiType build(String path) {
				String sidebar = "- [" + this.title + "](" + path + "/)\n" +
						wikis.stream()
								.map(wiki -> generateMdSidebar(wiki, path + "/" + wiki.name, 0))
	...
	public static final class Builder {
		...
		public WikiStructure build(String wikiPath) {
			...
			String masterSidebar =
					"- Versions:\n"
							+ versions.stream().map(wikiType -> "\t- [" + wikiType.title + "](" + wikiPath + "/versions/" + wikiType.name + "/)")
							.collect(Collectors.joining("\n"))

							+ "\n- Libraries:\n"
							+ libraries.stream().map(wikiType -> "\t- [" + wikiType.title + "](" + wikiPath + "/libraries/" + wikiType.name + "/)")
							.collect(Collectors.joining("\n"));
			...
			return new WikiStructure(
					...
					renderedSidebar.substring(
							renderedSidebar.indexOf("\n") + 1,
							renderedSidebar.lastIndexOf("\n") + 1
					)
			);
		}
	}
