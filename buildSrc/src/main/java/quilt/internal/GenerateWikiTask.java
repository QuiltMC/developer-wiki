...
import quilt.internal.templates.DateNowExtension;
...
public class GenerateWikiTask extends DefaultTask {
	...
	public GenerateWikiTask() {
		...
		loader.setPrefix(
				getProject()
						.getRootDir()
						.toPath()
						.resolve("wiki/templates/")
						.toString()
		);

		engine = new PebbleEngine.Builder()
				.extension(DateNowExtension.getExtension())
				.loader(loader)
				.autoEscaping(false)
				.build();
	...
	public void generateWiki() throws IOException {
		...
		generateStaticPage(
				...
				"Page not found",
				"The Quilt Developer Wiki.",
				getProject().property("wiki_path") + "/404"
		);
	...
	private void outputFile(
		...
		if (Files.exists(tree.path())) {
			...
			PebbleTemplate compiled = engine.getTemplate("tutorial_page.html");
			...
			String outputUrl = getProject()
					.property("wiki_path") + current.toString().replace("\\", "/")
					.replaceAll(".+/" + getProject().property("output_path"), "");
	...
	private void compileHtmlFile(Map<String, Object> defaultOptions, Path output, String body, String title, String url, String description) throws IOException {
		PebbleTemplate compiled = engine.getTemplate("master_template.html");
